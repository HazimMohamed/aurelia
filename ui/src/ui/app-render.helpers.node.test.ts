...
describe("resolveSessionDisplayName", () => {
  ...
  it("uses a friendly fallback for UUID-like keys", () => {
    expect(resolveSessionDisplayName("b927716e-b5dc-4dc0-a3e9-0a00f992f956")).toBe("Chat Session");
  });

  // ── With row data (name / displayName) ──────────
  ...
  it("returns parsed fallback when label matches key", () => {
    expect(resolveSessionDisplayName("mykey", row({ key: "mykey", name: "mykey" }))).toBe("mykey");
  });
  ...
  it("uses label alone when available", () => {
    expect(
      resolveSessionDisplayName(
        "discord:123:456",
        row({ key: "discord:123:456", name: "General" }),
      ),
    ).toBe("General");
  });
  ...
  it("prefers label over displayName when both are present", () => {
    expect(
      resolveSessionDisplayName(
        "discord:123:456",
        row({ key: "discord:123:456", displayName: "My Chat", name: "General" }),
      ),
    ).toBe("General");
  });
  ...
  it("ignores whitespace-only label and falls back to displayName", () => {
    expect(
      resolveSessionDisplayName(
        "discord:123:456",
        row({ key: "discord:123:456", displayName: "My Chat", name: "   " }),
      ),
    ).toBe("My Chat");
  });
  ...
  it("uses parsed fallback when whitespace-only label and no displayName", () => {
    expect(
      resolveSessionDisplayName("discord:123:456", row({ key: "discord:123:456", name: "   " })),
    ).toBe("Discord Session");
  });

  it("trims label and displayName", () => {
    expect(resolveSessionDisplayName("k", row({ key: "k", name: "  General  " }))).toBe("General");
  ...
  it("prefixes subagent label with Subagent:", () => {
    expect(
      resolveSessionDisplayName(
        ...
        row({ key: "agent:main:subagent:abc-123", name: "maintainer-v2" }),
      ),
  ...
  it("prefixes cron label with Cron:", () => {
    expect(
      resolveSessionDisplayName(
        ...
        row({ key: "agent:main:cron:abc-123", name: "daily-briefing" }),
      ),
  ...
  it("does not prefix non-typed sessions with labels", () => {
    expect(
      resolveSessionDisplayName(
        ...
        row({ key: "agent:main:bluebubbles:direct:+19257864429", name: "Tyler" }),
      ),
    ).toBe("Tyler");
  });
});
