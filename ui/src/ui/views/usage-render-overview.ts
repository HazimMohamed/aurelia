...
function renderFilterChips(
  ...
  const sessionsLabel = selectedSession
    ? (selectedSession.name || selectedSession.key).slice(0, 20) +
      ((selectedSession.name || selectedSession.key).length > 20 ? "…" : "")
    : selectedSessions.length === 1
      ? selectedSessions[0].slice(0, 8) + "…"
      : `${selectedSessions.length} sessions`;
  const sessionsFullName = selectedSession
    ? selectedSession.name || selectedSession.key
    : selectedSessions.length === 1
      ? selectedSessions[0]
