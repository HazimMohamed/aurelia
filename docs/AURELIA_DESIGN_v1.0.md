...
---
# TODO: Get rid of sections 1.0, 1.1 1.2. My reasoning is that we aren't necessarily sticking to this design right now. This is more of a stretch goal that doesn't pertain to the current philosophy of the application, instead it's how the application is intended to grow in the future. Maybe instead just add this to something akin to stretch goals.

**DECISION:** Agree. Moving §1 (The Council), §1.1, §1.2 to ROADMAP.md as aspirational agent designs. The current system is agent-agnostic — any agent can be created with any constitution. These specific five agents are a future goal, not a shipped feature.
...
# TODO: Maybe we can put this and bardo subprocess into a bigger section labeld it Agent life cycle

**RESPONSE:** Agree. In the rewritten DESIGN.md, consolidate §2 (Technical Notes) and §3 (Incarnation Lifecycle) into a single "Agent Lifecycle" section covering: spawn → cycles → hooks → bardo → dissolution.

# TODO: Let's discuss a better naming system. Maybe we can seperate incarnation display name and incarnation ID? Not sure where to go with this.

**RESPONSE:** Current system works well enough for MVP. The format `{agent}-{adjective}-{noun}-{number}` serves as both ID and display name. If we need human-friendly aliases later (e.g., "that conversation about teaching"), we can add a `name` field to incarnation metadata. Not urgent — defer to post-M4.
...
# TODO: This is explicitly not true anymore! We don't allow agents to read their own akashic records because they'll reflexively do this at each start up and waste tokens.

**DECISION:** Correct, this contradicts current reality. Agents **cannot** read their own Akashic records directly via bash anymore — this was disabled because agents would reflexively read them at startup and waste tokens. Akashic records are read-only archives managed by infra. Agents access memory through: (1) semantic/episodic core (always loaded in context), (2) `search_episodic` tool for targeted retrieval from extended memory.
...
# TODO: Maybe a mermaid or other graphics table would be worth adding here instead of a markdown. I want to discuss.

**RESPONSE:** A mermaid diagram would be clearer, but not critical for MVP. ASCII flowchart is fine for now — readable in plain text, works in GitHub. If we add visual diagrams later, keep the ASCII version too for CLI/grep discoverability. Low priority.

**ZUZU:** Okay at least preserve the TODO in the new doc
...
TODO: Are we doing this anymore?

**RESPONSE:** Yes, this is implemented. See `src/memory/bardo.py::run_bardo` lines 286-308. Memory flags with `tier: "semantic_core"` are processed via `_ensure_semantic_core()`.
...
# TODO: We call karma just memory now

**DECISION:** Ack. Update all references: `~/karma/` → `~/memory/` (or keep karma in paths but call it "memory" in prose). Actually, looking at `config.py`, the paths are `config.karma_dir` but aliased as `config.memory_dir`. Use "memory" consistently in documentation.

Karma incarnation folder cleaned
Current symlink removed
    ↓
# TODO: I don't think we

**RESPONSE:** (Incomplete TODO — ignoring.)
...
# TODO: This should probably be merged with the above section as Agent Lifecycle with each hook type as a unique entry way into the lifecycle

**DECISION:** Agree. Merge §2 and §3 into unified "Agent Lifecycle" section in the rewrite. Structure: Overview → Spawn → Hooks (4 types as entry points) → Cycles → Bardo → Dissolution.
...
# TODO: Maybe we should combine the section on heartbeats and scheduled tasks since heartbeats are a sub-type of scheduled task.

**RESPONSE:** Conceptually yes, but pedagogically no. Heartbeats deserve their own explanation because they have unique behavior (precheck, exploration-focused). Keep them separate in the docs but note that heartbeats are implemented as recurring scheduled tasks under the hood.
...
# TODO: What is this talking about? I understand that Sonnet will just continue to use tools until it completes the cycle. Are heartbeats necessarily one cycle? I think yes right?

**RESPONSE:** Correct — heartbeats are one cycle. The agent wakes, does its work (which may include multiple tool calls in the same LLM turn), returns a response, and signals `done`. The confusing part was the phrase "decides whether to end quickly" — clarify that heartbeat_precheck happens *before* the cycle starts. If precheck fails, no cycle runs at all.
...
# TODO: THe first two lines of this section seem to not really pertain to anything else in this section

**RESPONSE:** Agree, the section title doesn't match the content. This section is really about "Bardo Triggers and Smart Gating." The "Next Action Format" detail (JSON at end of response) should be moved to §3.2 or a tool description. Reorganize in rewrite.
...
# TODO: Let's avoid mentioning experiments in this document outside of a dedicated experiment section because it's not really a production feature.

**DECISION:** Agree. Remove experiment references from DESIGN.md. Experiments (`lab/`, `sandbox/`) should get a brief §Experiments or be documented separately in `lab/README.md`. The design doc describes production architecture only.

- **Experiment end:** `AureliaExperiment.__exit__` calls `trigger_bardo` explicitly
- **CLI:** `aurelia agent bardo <name>` or direct runtime call
...
# TODO:  This section seems sparse of novel information. Maybe we should scratch it.

**DECISION:** Agree. §3.4 (Incarnation Policy) is redundant with the hook descriptions above and the runtime interface below. Delete this section. The flowchart doesn't add clarity — it's already explained in §3.1 and §7.
...
# TODO: This section seems redundant with sections above. We should probably delete it too.

**DECISION:** Agree. §3.5 (Bardo Triggers) repeats information from §3.3. Consolidate into a single "Bardo" subsection covering: what it is, when it triggers, smart gating (worth_archiving/worth_consolidating), and the full process flow.
...
# TODO: Is this implented as such yet?

**RESPONSE:** Yes, budget exhaustion is implemented in `src/agent/budget.py::check_and_apply_budget`. When budget is exhausted: (1) current cycle completes, (2) status set to "budget_paused", (3) dashboard notification written, (4) future cycles blocked until manual override. See lines 117-129 in budget.py.
...
def undissolve(agent: str, incarnation_name: str):
    ...
    # TODO: Karma is now called memory. Fix elsewhere.

    **DECISION:** Ack. Paths are `/var/aurelia/agents/{agent}/memory/` in code (`config.karma_dir`). Update all doc references to say "memory" not "karma" except when showing actual filesystem paths.
    ...
    # TODO: This should be an explicit flag to be passed into reincarnated agents

    **RESPONSE:** Good idea. Add an `undissolved: true` flag to the incarnation_start entry so the agent knows this is a restored session, not a fresh spawn. Not critical for MVP but useful for agent self-awareness. Add to ROADMAP or post-M4 improvements.
...
# TODO: The directory structure is actually split between the data  directory and the hoem directory, so that the agent can't access it's own itnernal state files like agent.json and such.

**DECISION:** Critical correction. The actual structure is:
- `/home/{agent}/` — agent-owned workspace (room/, dharma/, identity/)
- `/var/aurelia/agents/{agent}/` — samsara-managed data (memory/, akasha/, agent.json)
- `/var/aurelia/run/{agent}/` — runtime state (manas.sock, manas.pid)

This split prevents agents from reading/modifying their own agent.json, budget files, or Akashic archives. Must document accurately in §4.1 and §6.4.
...
# TODO: I think we're not very explicit about the difference between semantic and episodic memory. Would you consider collapsing these two, both in code and in the docs? I understand the underlying intention but for the sake of simplicity wouldn't a single "memory" suffice for now. Something to think about.

**RESPONSE:** The distinction is philosophically important (facts vs experiences, wisdom vs autobiography) but you're right that we haven't justified it clearly. For MVP, keep both but add a clear explanation: Semantic = generalizable knowledge (passes the six-month test). Episodic = specific experiences (dated, narrative). If this distinction proves useless in practice, we can collapse them post-M4. The split enables future capabilities like "show me formative moments" vs "what do you know about X?"
...
# TODO: Is this implemented? If not I would just kill it from the doc. This isn't really a high level architecture detail more like an implementation detail.

**RESPONSE:** Yes, fcntl file locking is implemented. See `src/memory/memory.py::write_semantic_core` lines 81-97. But you're right — this is an implementation detail. Move to a footnote or remove from the main design doc. The important part is "writes are atomic and safe for concurrent access."
...
# TODO: In the spirit of the YAGNI principle let's 86 this for now. It serves a real practical function but its not central to the core product we're tring to launch.

**DECISION:** Agree. Move §4.4 (Traumatize/Enshrine) to ROADMAP.md under M4 (FE features). These are valuable but unimplemented. For now, episodic core promotion happens manually via Janitor CLI.
...
# TODO: What is it talking about with outcomes.jsonl I don't remember this.

**RESPONSE:** outcomes.jsonl was a proposed file for tracking agent accomplishments, scrapped before implementation. Delete this paragraph — it references a design that never shipped.
...
# TODO: Again this is not true anymore because of the data dir/home dir split.

**DECISION:** Correct — §4.6 shows old single-home structure. Rewrite to reflect data/home split: `/home/{agent}/` is agent-owned 700, `/var/aurelia/agents/{agent}/` is aurelia-owned with selective read permissions for agents via Manas process running as agent user.
...
# TODO: Let's point people towards setting this stuff up via the CLI. If the CLI doesn't support it, add CLI support to TODO.md.

**RESPONSE:** CLI already supports agent creation via `aurelia agent create <name>`. This calls `src/samsara/provisioning.py::create_agent()` which handles all setup. Update docs to mention: "Agent provisioning: `sudo aurelia agent create <name>` — creates user, directories, sets permissions, registers with runtime."
...
# TODO: This is now explicitly forbidden as the agent will reflexively read it's own information whcih is bad. For more information look at:

**DECISION:** Correct — agents CANNOT read their own memory directly via bash anymore. They waste tokens doing reflexive reads at startup. Memory access is now controlled: (1) semantic/episodic core always loaded in system prompt, (2) `search_episodic` tool for targeted queries, (3) bash access to `/home/{agent}/room/` and scratch only. Delete the "bash remains the primary read path" claim.
...
# TODO: This is the true version and it contradicts a lot of what is already in the doc.

**DECISION:** Agree — this section is accurate. Use this as the canonical reference for agent capabilities and update contradicting sections elsewhere.
...
# TODO: I like this. I think we should definitely mention it in the the TODO.md.

**RESPONSE:** Ack. This is implemented and working. Keep in DESIGN.md as-is — it's a key capability. The `search_episodic` tool is how agents access historical context without reflexive bash reads.
...
# TODO: Why is the bardo process described like in 10 different places? Maybe consildate this?

**DECISION:** Absolutely right — bardo is explained in §2 (flowchart), §3.3 (triggers), §3.5 (triggers again), and §4.8 (process). In the rewrite, consolidate into ONE definitive "Bardo" subsection covering: what it is, when it triggers, smart gating, and the full process. Reference it elsewhere rather than repeating.
...
# TODO: Let's stick to the YAGNI principle and 86 this for now.

**DECISION:** Agree. Move §5.3 (Stack Maintenance) to ROADMAP.md. Automated curation is nice-to-have but not MVP. For now, the shared context stack is append-only with manual pruning if needed.
...
# TODO: This section is much more recent and therefore much more authoritative on how the app actually works today.

**RESPONSE:** Correct — §6 is accurate and reflects shipped architecture. Use this as the canonical reference. Earlier sections (especially §1-4) have stale information that contradicts §6. When rewriting, start from §6 as the source of truth.
...
# TODO: I don't think we're actually using the transport group. Maybe more discussion and possibly an entire section is needed to explain the permission model that overlays the directory structure.

**RESPONSE:** Good catch. With Manas, the transport_group concept is less relevant — each agent has its own Manas process running as that agent user, and the runtime daemon routes via Manas sockets. The permission model is now: (1) Runtime daemon (aurelia user) routes to Manas, (2) Manas (agent user) runs agent code, (3) Transports (HTTP/CLI) connect to runtime daemon socket. Needs clarification in the rewrite, especially how Manas eliminates the need for sudo in bash_exec.
...
# TODO: With the new Manas agent addition this no longer needs to use sudoers or sudo -u. It naturally runs with Manas (agent-level) permissions

**DECISION:** CRITICAL UPDATE. With Manas architecture, bash_exec runs directly as the agent user (Manas process is already that user). No sudo needed. The sudoers entry is now ONLY for backward compatibility or manual ops. Update this entire section to reflect Manas as the primary execution model.
...
# TODO: This section is definitely redundant with a very similar one above.

**DECISION:** Agree — §6.4 repeats §4.1 with slight variations. In the rewrite, consolidate into ONE canonical "Directory Structure" section showing the data/home split clearly. Include both the logical view (what each dir is for) and the physical layout (actual paths).
...
# TODO: I'm not sure I understand the use case for both cases and why sockets can't just handle everything. I think we need to discuss this.

**RESPONSE:** The FIFO queues are legacy from M2 when we had separate per-agent daemon processes. With Manas, everything routes through sockets: runtime.sock → runtime daemon → manas.sock → Manas process. FIFOs are kept for backward compatibility but aren't the primary path anymore. We should clarify this or remove FIFO references entirely if we're committed to Manas-only.
...
# TODO: I this we actually deviated from the naming here a little bit. I think we do somethign like sudo aurelia start --http rather than aurelia serve http. Look into this.

**RESPONSE:** Need to verify the actual CLI. If it's `aurelia start --http`, update this section. If both work, document the canonical form. Check `cli/aurelia` to see what's actually implemented.
...
**RESPONSE:** Partially stale. The permissions are directionally correct but don't reflect Manas. With Manas, the agent process (Manas running as agent user) has natural access to what it needs — no sudo gymnastics. Update this section to show Manas-based permissions, not the old sudo -u model.
...
```
# TODO: how about we just get rid of references to aurelia-admin

**DECISION:** Agree. Remove all mentions of "aurelia-admin" endpoints/auth. Admin operations go through: (1) CLI (`aurelia` commands), (2) Janitor agent (for operations requiring agent context/reasoning). This is the shipped reality.
...
# TODO lets 86 this janitor references since i want it to be something more like a stretch goal

**DECISION:** Agree. The Janitor is aspirational, not shipped. Remove Janitor-specific types from the core design doc. Document the type whitelist as-is (available to all agents), and note that elevated operations (registry_reload, etc.) are future work. Move Janitor concept to ROADMAP.md.
...
# TODO why route through http? cant we hit the socket endpointr directly?

**RESPONSE:** Good question. Agent tools currently call HTTP endpoints because that's what was built first. In principle, tools could call the runtime socket directly (just like transports do). This would eliminate HTTP as a middleman for agent→scheduler communication. Worth considering for a future refactor, but not urgent — HTTP works fine and is already authenticated via bearer tokens.
...
# TODO Can we discuss a possible use case for this tool?

**RESPONSE:** The use case is explicit multi-step work: "I need to do X, Y, and Z — let me call continue_task after each step so I stay in the same context window." In practice, agents rarely need this because they can just keep calling tools in the same turn. The tool exists for theoretical clarity (incarnation = context window = lifecycle) but may be vestigial. Monitor actual usage; consider deprecating if never used.
...
# TODO: Lets rewrite these

**DECISION:** Agree. §11 (The Milestones) is historical changelog material, not design documentation. Move completed milestones (M1-M3) to CHANGELOG.md with dates/commits. Move future milestones (M4-M5) to ROADMAP.md with clear "not yet implemented" warnings. Keep design doc focused on what IS, not what WAS or WILL BE.
